...
var ProcessInputStep2 = (List<string> lines) =>
{
    ...
    while (!found)
    {
        ...
        if(res % 1000 == 0) PrintProgress(stepNo, res, current);

        for(var x = 0; x < current.Count(); x++)
        {
            if (step == 'L')
            {
                current[x] = collection.Nodes.FirstOrDefault(q => string.Equals(q.Node, current[x], StringComparison.InvariantCultureIgnoreCase))?.LeftNode;
            }

            if (step == 'R')
            {
                current[x] = collection.Nodes.FirstOrDefault(q => string.Equals(q.Node, current[x], StringComparison.InvariantCultureIgnoreCase))?.RightNode;
            }
        }

        if (current.All(q => q[2] == 'Z')) found = true;
    }
